import pygame, sys, os, time, math, random, json, functools, asyncio
...
async def launch_mode(picked):
    ...
    if picked in ("single", "dual"):
        ...
        # [優化] Web 版改為直接呼叫整合後的非同步模組
        try:
            pygame.mixer.music.pause()
            # 傳入目前的 screen 與 clock
            await run_heimlich(screen, clock)
            
            # 返回後恢復
            pygame.mixer.music.unpause()
            pygame.display.set_mode((WIDTH, HEIGHT), pygame.RESIZABLE | pygame.WINDOWMAXIMIZED)
            state = "mode_select"; menu_sel = 8
        except Exception as e:
            print(f"啟動模擬器失敗: {e}")
        return True
...
# ============================================================
async def main():
    global running, state, prev_state, dt, now, anim_t, tut_anim, current_bgm, shake_timer, shake_amount
    global WIDTH, HEIGHT, screen, bg, menu_bg_img, rhy_bg_img, bpm_bg_img, emei_aed_bg_img, overlay_surf, f_ov_surf, temp_screen
    global menu_sel, game_mode, play_mode, buf, time_lim, round_goal, prepare_t, finish_t
    ...
    while running:
            ...
            if e.type == pygame.QUIT:
                ...
                # ------ mode_select ------
                    ...
                    if e.key == pygame.K_UP:
                        ...
                        await launch_mode(picked)   # [優化] 統一入口，消除重複的 if/elif 連鎖
        ...
        pygame.display.flip()
        await asyncio.sleep(0) # 關鍵：釋放 CPU 給瀏覽器渲染
