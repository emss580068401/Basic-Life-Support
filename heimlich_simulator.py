...
def rrect(surf, col, rect, r, bw=0, bc=None, alpha=255):
    ...
    if len(_RRECT_CACHE) > 1000: _RRECT_CACHE.clear()
...
def glow(surf, col, cx, cy, r, steps=4, a0=55):
    ...
    if len(_GLOW_CACHE) > 200: _GLOW_CACHE.clear()
...
class BeatBar:
    ...
    def reset(self,obstacle_id):
        speeds={"candy":0.68,"meat":0.90,"mochi":1.15}
        self.speed=speeds.get(obstacle_id,0.80)
...
class Game:
    def __init__(self):
        ...
        self.particle_group = pygame.sprite.Group() # 【效能優化】取代陣列，使用 Sprite 群組
        self.popups=[]
    ...
    def _reset_vars(self):
        ...
        self.oxygen=100.0; self.global_progress=0
        self.slap_count=0; self.thrust_count=0
        self.global_progress=0.0; self.is_charging=False; self.power=0.0; self.last_hovered_btn=None
...
class SimulationRenderer:
    ...
    def draw_body_panel(surf, g):
        ...
        if g.is_charging and g.stage == "front":
            ...
            jet = pygame.Surface((s_bw, s_jet_h), pygame.SRCALPHA)
            for step in range(5): 
                pygame.draw.rect(jet, (*C["cyan"], int(60 * (g.power / 100) * (step / 4))), (0, LogicalScreen.s(step * 12), s_bw, LogicalScreen.s(12)))
            # 將氣流 blit 到轉換後的螢幕座標
            jet_pos = to_s(0, ay + ah - 20)
            surf.blit(jet, jet_pos)
        ...
        if g.hit_flash > 0: 
            flash_surf = pygame.Surface((bw_s, bh_s), pygame.SRCALPHA)
            flash_surf.fill((*C["emerald"], int(g.hit_flash / 0.18 * 40)))
            surf.blit(flash_surf, (bx_s + sx, by_s + sy), special_flags=pygame.BLEND_RGBA_ADD)
    ...
    def draw_overlays(surf, g, mx, my):
        ...
        if g.fail_flash > 0:
            ...
            overlay.fill((*C["red"], int(g.fail_flash / 0.45 * 80))); surf.blit(overlay, (0, 0))

        if g.state in ("victory", "gameover"):
            dim = pygame.Surface(surf.get_size(), pygame.SRCALPHA)
            dim.fill((4, 9, 22, 220)); surf.blit(dim, (0, 0)) # 加深背景
    ...
    def draw_phase_bar(surf, g):
        ...
        msg_font = load_font(int(38 + 6 * math.sin(g.t * 8)), True) if g.is_spot_found else F22
    ...
    def draw_start_screen(surf, real_mx, real_my, t, g):
        ...
        # 模式按鈕垂直堆疊布局 (Y: 535, 615 - 稍微下移)
        btn_configs = [
            ("一般模式", "standard", C["cyan"], LogicalScreen.s(535)),
            ("肥胖者及孕婦", "pregnant", C["fuchsia"], LogicalScreen.s(615))
        ]
